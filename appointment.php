...
<section class="booking_section">
	<div class="container">
		...
		<?php
            ...
            if(isset($_POST['submit_book_appointment_form']) && $_SERVER['REQUEST_METHOD'] === 'POST')
            {
                ...
                try
                {
					...
					{
						$stmtgetCurrentClientID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'beauty_salon' AND TABLE_NAME = 'clients'");
                    ...
                    $stmtgetCurrentAppointmentID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'beauty_salon' AND TABLE_NAME = 'appointments'");
                    ...
                    echo "<div class = 'alert alert-success'>";
                        echo "Ваш запис було успішно створено";
                    echo "</div>";
		...
		<form method="post" id="appointment_form" action="appointment.php">
          ...
          Будь ласка, виберіть хоча б одну послугу!
				</div>
                                    	...
                                    	<div class="select_item_bttn">
                                    		<div class="btn-group-toggle" data-toggle="buttons">
												<label class="service_label item_label btn btn-secondary">
													<input type="checkbox"  name="selected_services[]" value="<?php echo $row['service_id'] ?>" autocomplete="off">Обрати
												</label>
											</div>
                                    	</div>
                                    <?php
			...
			<div class="select_employee_div tab_reservation" id="employees_tab">
				...
				<div class="alert alert-danger" role="alert" style="display: none">
					Будь ласка, виберіть свого майстра!
				</div>
                                    ...
                                    ?>
                                    		<div class="select_item_bttn">
                                    			<label class="item_label btn btn-secondary active">
													<input type="radio" class="radio_employee_select" name="selected_employee" value="<?php echo $row['employee_id'] ?>">Обрати
												</label>
                                    		</div>
                                    <?php
                                		echo "</div>";
			...
			<div class="client_details_div tab_reservation" id="client_tab">
                ...
                <div>
                    <div class="form-group colum-row row">
                        <div class="col-sm-6">
                            <input type="text" name="client_first_name" id="client_first_name" class="form-control" placeholder="Ім'я">
							<span class = "invalid-feedback">Це поле є обов'язковим</span>
                        </div>
                        <div class="col-sm-6">
                            <input type="text" name="client_last_name" id="client_last_name" class="form-control" placeholder="Прізвище">
							<span class = "invalid-feedback">Це поле є обов'язковим</span>
                        </div>
                        <div class="col-sm-6">
                            <input type="email" name="client_email" id="client_email" class="form-control" placeholder="E-mail">
							<span class = "invalid-feedback">Недійсна електронна адреса</span>
                        </div>
                        <div class="col-sm-6">
                            <input type="text"  name="client_phone_number" id="client_phone_number" class="form-control" placeholder="Номер телефону">
							<span class = "invalid-feedback">Недійсний номер телефону</span>
						</div>
                    </div>
			...
			<div style="overflow:auto;padding: 30px 0px;">
    			<div style="float:right;">
    				<input type="hidden" name="submit_book_appointment_form">
      				<button type="button" id="prevBtn"  class="next_prev_buttons" style="background-color: #bbbbbb;"  onclick="nextPrev(-1)">Попередня</button>
      				<button type="button" id="nextBtn" class="next_prev_buttons" onclick="nextPrev(1)">Наступна</button>
    			</div>
  			</div>
